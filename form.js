...
async function cargarDatosFormulario() {
  ...
  onValue(formQuery, (snapshot) => {
    if (snapshot.exists()) {
      ...
      // Store minAge and maxAge
      currentMinAge = specificFormData.minAge !== undefined ? specificFormData.minAge : null;
      currentMaxAge = specificFormData.maxAge !== undefined ? specificFormData.maxAge : null;
...
if (formData) {
  formData.addEventListener('submit', async (event) => {
    ...
    // Age validation against form's minAge and maxAge
    ...
    window.datosParaConfirmar = { nombre, cedula: cedulaRaw, edad: edadValue }; // Use edadValue (string) as it's used later
  });
}
...
if (btnConfirmar) {
  btnConfirmar.addEventListener('click', async function () {
    ...
    const nuevaRespuesta = {
      ...
      cedula,
      edad, // This should be the numeric age for consistency if stored as number in Firebase
    ...
    try {
      ...
      outEdad.textContent = `${edad} años`; // Display age with "años"
...
if (btnCorregir) {
  btnCorregir.addEventListener('click', () => {
    confirmacionDatos.style.display = 'none';
    if (formData) {
      // Ensure form is displayed only if it was hidden and formId is valid
